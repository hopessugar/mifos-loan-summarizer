...
export function RiskBadge({ risk }) {
  ...
  const label = bps < 50 ? t('risk.lowProtection') : bps < 80 ? t('risk.moderateProtection') : t('risk.highProtection')
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
        <div>
          <div style={{ fontSize: '11px', color: '#999', marginBottom: '2px', textTransform: 'uppercase' }}>{t('risk.bpsLabel')}</div>
        ...
        <div style={{ flex: 1 }}>
          ...
          {risk.factors?.[0] && (
            <div style={{ fontSize: '12px', color: '#888', marginTop: '6px' }}>
              {t('risk.primaryIssue')} {risk.factors[0]}
            </div>
          )}
        </div>
      </div>
      ...
      {risk.negotiation_tips && risk.negotiation_tips.length > 0 && (
        <div style={{ marginTop: '8px', paddingTop: '12px', borderTop: '1px solid #F0F0EE' }}>
          <div style={{ fontSize: '12px', fontWeight: '600', color: '#333', marginBottom: '8px' }}>{t('risk.negotiationTips')}</div>
